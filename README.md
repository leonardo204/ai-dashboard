...
| 경로 | 용도 |
| --- | --- |
| `POST /v1/ai` | 채팅·비전·웹검색 (예전 경로 `/v1/recognize`도 받음) |
...
`ai.zerolive.co.kr` 은 이 코드가 분리되어 나온 **기존 worker(`hamzzi-diet`)에 아직 붙어 있습니다.** 그 상태로 배포하면 도메인 충돌로 실패합니다. 순서는 이렇습니다.
...
- **D1은 기존 프록시가 쓰던 데이터베이스를 그대로 씁니다.** 이름이 `hamzzi-stats`인 건 분리 전 흔적일 뿐이고, 바인딩은 id 기준이라 동작에는 영향이 없습니다. 지금까지 쌓인 호출 기록과 등록된 앱이 그대로 보입니다.
