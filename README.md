...
docs/           API 가이드 · 연결 가이드 원문
scripts/        가이드 원문을 코드로 옮기는 스크립트
```
...
| 가이드 | 다른 프로젝트에 건네줄 연결 가이드. 화면에서 읽고 `.md` 원문으로 내려받습니다. |
...
`가이드` 탭은 `docs/PROXY-CLIENT-GUIDE.md` 를 그대로 그립니다. 화면에 보이는 것과 내려받는 파일이 언제나 같습니다.

- Workers에는 파일 시스템이 없어서, `npm run gen:guide` 가 원문을 `src/guide-md.ts` 로 옮겨 적습니다. `typecheck`·`dry-run`·`deploy` 앞에서 자동으로 돌아가므로 따로 부를 일은 거의 없습니다.
- **문서를 고칠 때는 `docs/PROXY-CLIENT-GUIDE.md` 만 고칩니다.** `src/guide-md.ts` 는 자동 생성 파일입니다.
- 이 가이드에는 관리자용 내용(앱 등록·토큰 발급·통계 조회·앱 관리 API·서버 구성)을 넣지 않습니다. 받는 쪽은 앱 id와 토큰만 있으면 연결됩니다.
